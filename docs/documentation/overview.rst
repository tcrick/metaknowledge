...
The <a href="#RecordCollection">metaknowledge.RecordCollection</a> class can take a path to one or more of these files load and parse them. The object is the main way for work to be done on multiple records. For each individual record it creates an instance of the <a href="#Record">metaknowledge.Record</a> class that contains the results of the parsing of the record.
...
Citations are handled by a special <a href="#Citation">Citation</a> class. This class can parse the citations given by <em>WOS</em> and journals cited by *Scopus* and allows for better comparisons when they are used in graphs.

Note for those reading the docstrings metaknowledge’s docs are written in markdown and are processed to produce the documentation found at <a href="/metaknowledge/documentation/">networkslab.org/metaknowledge/documentation</a>, but you should have no problem reading them from the help function.
